...
/*
 * Test that a string returns a pattern
 *
 * @param {Object} options application options
 * @param {String} patternPath path to pattern
 *
 * @return {Object} an empty stream
*/
exports.testIfSinglePattern = function (options, patternPath) {
  ...
  // check that the patternPath ends with the dataFileName (pattern.yml); if not, add it to the end
  if(path.basename(patternPath) !== projectOptions.dataFileName){
    patternPath = path.join(patternPath, projectOptions.dataFileName);
  }

  try {
    var stats = fs.statSync(patternPath);
    console.log("Pattern data file exists.");
  }
  catch (e) {

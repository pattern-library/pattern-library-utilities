...
exports.getPatternImportData = function (paths, options) {
  ...
  if(patternTemplate){
    ...
    if(options.importStyles){
      ...
      if(patternStyle){
        ...
        patternStyle.forEach(function (style) {
          ...
          var patternFileToCopy = {
            type: 'styles',
            src: path.join(paths.folder,style.name),
            dest: path.join(patternFiles.patternStylesPath,style.name)
          }
    ...
    if(patternObject.script && options.importScripts){
      ...
      // iterate over an array of scripts, add each one to filesToCopy
      scriptArray.forEach(function (script) {
        // get the relative path to the js file source
        var jsFileSourcePath = path.join(paths.folder,script.name);

        // get the relative path to the js file destination
        var jsFileDestinationPath = path.join(patternFiles.patternScriptsPath,script.name);
